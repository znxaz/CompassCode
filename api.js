import { updateOptions } from "./skills.js";
...
export function setSkillStatus() {
  ...
  const apiURL = "http://localhost:8080/updateStatus";
...
export function fetchCourseStatuses() {
  fetch("http://localhost:8080/getStatus")
      ...
      .then(text => {
          try {
              const data = JSON.parse(text);
              if (data.error) {
                  console.error("Error fetching course statuses:", data.error);
              } else {
                console.log(data);
                  updateOptions(data);
              }
